...
test('revised pilot has the intended mix and no repeating source-key rotation',()=>{
 assert.equal(bank.questions.filter(q=>q.questionStyle==='scenario').length,13);
 assert.equal(bank.questions.filter(q=>q.questionStyle==='concept').length,27);
 const keys=bank.questions.map(q=>q.c[0]);
 for(let period=1;period<=10;period++)assert.ok(keys.some((key,i)=>i>=period&&key!==keys[i%period]),`period-${period} key pattern`);
 for(const q of bank.questions)assert.equal(q.rationales.length,q.o.length,q.id);
});
test('cold-review packet is current and excludes answer-bearing metadata',()=>{
 const {renderPacket}=require('./build-cdmp-review');
 const rendered=renderPacket(bank);
 assert.equal(fs.readFileSync(path.join(__dirname,'../cdmp/BLIND_PILOT.md'),'utf8'),rendered);
 assert.ok(rendered.includes(E.bankFingerprint(bank)));
 assert.equal((rendered.match(/^## \d+\. CDMP-P-/gm)||[]).length,40);
 // Deliberate sentinel values prove only stem/option fields reach the packet.
 const fixture=JSON.parse(JSON.stringify(bank));
 fixture.questions[0].e='SECRET_EXPLANATION';fixture.questions[0].rationales=['SECRET_RATIONALE'];
 fixture.questions[0].sources=[{title:'SECRET_SOURCE',url:'https://example.test/SECRET_URL'}];
 fixture.questions[0].family='SECRET_FAMILY';fixture.questions[0].c=['SECRET_KEY'];
 assert.doesNotMatch(renderPacket(fixture),/SECRET_/);
});
